...
const Layout = ({ children }) => {
  ...
  const navigation = [
    ...
    { name: 'Anomalies', href: '/app/anomalies', icon: AlertTriangle },
      ...
      <div className={`fixed inset-0 z-50 lg:hidden ${sidebarOpen ? 'block' : 'hidden'}`}>
        ...
        <div className="fixed inset-y-0 left-0 flex w-64 flex-col bg-white shadow-xl">
          <div className="flex h-16 items-center justify-between px-4">
            <h1 className="text-xl font-bold text-gray-900">Smart Claims</h1>
            <button
        ...
        <div className="sticky top-0 z-10 flex h-16 bg-white shadow lg:hidden">
          ...
          <div className="flex flex-1 items-center justify-center px-4">
            <h1 className="text-lg font-semibold text-gray-900">
              {navigation.find(item => item.href === location.pathname)?.name || 'Smart Claims'}
            </h1>
          </div>
        </div>
